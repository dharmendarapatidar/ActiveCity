...
from Ac_Citizen.citizen_serialize import CitizenSerialize, ComplaintSerialize, FeedbackSerialize
...
# Serialize Part
# For Citizen serializer
class CitizenGet(APIView):
    def get(self,request):
        qs = Citizen_Table.objects.all()
        obj = CitizenSerialize(qs,many=True)
        return Response(obj.data)


class CitizenPost(APIView):
    def post(self,request):
        obj = CitizenSerialize(data=request.data)
        if obj.is_valid():
            obj.save()
...
# For Complaint serializer
class ComplaintGet(APIView):
    def get(self,request):
        qs = Complaint_Table.objects.all()
        obj = ComplaintSerialize(qs,many=True)
        return Response(obj.data)


class ComplaintPost(APIView):
    def post(self,request):
        obj = ComplaintSerialize(data=request.data)
        if obj.is_valid():
            obj.save()
            return Response(obj.data,status=status.HTTP_201_CREATED)
        return Response(obj.errors,status=status.HTTP_400_BAD_REQUEST)
...
# For feedback serializer
class FeedbackGet(APIView):
    def get(self,request):
        qs = Feedback_Table.objects.all()
        obj = FeedbackSerialize(qs,many=True)
        return Response(obj.data)


class FeedbackPost(APIView):
    def post(self,request):
        obj = FeedbackSerialize(data=request.data)
        if obj.is_valid():
            obj.save()
            return Response(obj.data,status=status.HTTP_201_CREATED)
        return Response(obj.errors,status=status.HTTP_400_BAD_REQUEST)
