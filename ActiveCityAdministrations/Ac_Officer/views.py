...
# officer_feedback_list
def officer_feedback_list(request):
    try:
        username = request.session['username']
        dept = request.GET.get('name')
        qs = Feedback_Table.objects.filter(dept_name_id=dept)
        return render(request,'ac_officer/officer_feedbacks_list.html',{'list':qs})
    except KeyError:
        redirect('officer_login')


# officer_feedback_replay
def officer_feedback_replay(request):
    try:
        username = request.session['username']
        ...
        if request.method == 'POST':
            ...
            return render(request, 'ac_officer/officer_feedbacks_replay.html',
                          {'data': qs1, 'rep_message': 'Replay send successfully'})
        else:
            return render(request, 'ac_officer/officer_feedbacks_replay.html', {'data': qs1})
    except KeyError:
        return redirect('admin_login')